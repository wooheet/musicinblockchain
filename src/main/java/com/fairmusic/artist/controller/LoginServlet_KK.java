...
@WebServlet(name = "login_kk", urlPatterns = { "/login_kk.do" })
public class LoginServlet_KK extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("Ä«Åå ·Î±×ÀÎ¼­ºí¸´  code : "+ artist_code);
		System.out.println("Ä«Åå ·Î±×ÀÎ¼­ºí¸´  artist_code : "+ artist_code);
		System.out.println("Ä«Åå ·Î±×ÀÎ¼­ºí¸´ ÀÌ¸§ : "+name);
		System.out.println("Ä«Åå state : "+state);
		
		ArtistServiceimpl service = new ArtistServiceimpl();
		artistDTO user = service.getArtistDTO(artist_code);
		HttpSession ses = request.getSession();
		
		System.out.println("Ä«Åå ·Î±×ÀÎ¼­ºí¸´  user : "+ user);
